...
class ImageTransformer:
    ...
    @staticmethod
    def pierce(frame, piercesize, return_neg=False):

        # Declare useful parameters
        height, width, _ = frame.shape
        Dw, Dh = piercesize

        # Compute piercing limits
        ...
        # Perform "piercing" operation
        frame_p = cv.rectangle(frame.copy(),(x0,y0),(xf,yf),color=(255,255,255),thickness=-1)
        pierce = frame[y0:yf,x0:xf]

        '''
        frame_test = cv.line(frame.copy(),(x0,y0),(x0+Dw,y0),color=(255,0,0),thickness=2)
        frame_test = cv.line(frame_test,(x0+Dw,y0),(x0+Dw,y0+Dh),color=(255,0,0),thickness=2)
        frame_test = cv.line(frame_test,(x0+Dw,y0+Dh),(x0,y0+Dh),color=(255,0,0),thickness=2)
        frame_test = cv.line(frame_test,(x0,y0+Dh),(x0,y0),color=(255,0,0),thickness=2)
        '''

        if return_neg == False:
            return frame_p
        else:
            return frame_p, pierce
